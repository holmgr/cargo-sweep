...
fn remove_not_built_with_in_a_profile(
    ...
    let mut total_disk_space = 0;
    total_disk_space += remove_not_matching_in_a_dir(&dir.join("build"), &keep, dry_run, debug_output)?;
    total_disk_space += remove_not_matching_in_a_dir(&dir.join("deps"), &keep, dry_run, debug_output)?;
    // examples is just final artifacts not tracked by fingerprint so skip that one.
    // incremental is not tracked by fingerprint so skip that one.
    // `native` isn't generated by cargo since 1.37.0
    let native_dir = dir.join("native");
    if native_dir.exists() {
        total_disk_space += remove_not_matching_in_a_dir(&native_dir, &keep, dry_run, debug_output)?;
    }
    total_disk_space += remove_not_matching_in_a_dir(dir, &keep, dry_run, debug_output)?;
    total_disk_space += remove_not_matching_in_a_dir(&dir.join(".fingerprint"), &keep, dry_run, debug_output)?;
    Ok(total_disk_space)
}
...
pub fn remove_not_built_with(
    ...
    for fing in lookup_all_fingerprint_dirs(dir) {
        ...
        total_disk_space +=
            remove_not_built_with_in_a_profile(path.parent().unwrap(), &keep, dry_run, debug_output)?;
    }
    Ok(total_disk_space)
}
...
pub fn remove_older_until_fits(path: &Path, target_size: u64, dry_run: bool, debug_output: bool) -> Result<u64, Error> {
    ...
    for (fing, keep) in organized {
        total_disk_space +=
            remove_not_built_with_in_a_profile(fing.parent().unwrap(), &keep, dry_run, debug_output)?;
    }
